...
class GameUI:
    def __init__(self, screen, clock, actions_status):  # each player initialised with its data
        self.screen = screen
    ...
    def draw_actions_ui(self):
        ...
        self.launchDice = UIButton(
            relative_rect=pygame.Rect(ACTIONS_WIDTH // 2 - BUTTON_WIDTH // 2, 50, BUTTON_WIDTH, BUTTON_HEIGHT),
            text="Lancia i dadi",
        ...
        self.buyButton = UIButton(
            relative_rect=pygame.Rect(ACTIONS_WIDTH // 2 - BUTTON_WIDTH // 2, 100, BUTTON_WIDTH, BUTTON_HEIGHT),
            text="Compra",
            container=actions_UI,
            object_id="BUY",
            manager=self.manager,
        ...
        self.showStocks = UIButton(
            relative_rect=pygame.Rect(ACTIONS_WIDTH // 2 - BUTTON_WIDTH // 2, 150, BUTTON_WIDTH, BUTTON_HEIGHT),
            text="Mostra Cedole",
            container=actions_UI,
        ...
        self.passButton = UIButton(
            relative_rect=pygame.Rect(ACTIONS_WIDTH // 2 - BUTTON_WIDTH // 2, 200, BUTTON_WIDTH, BUTTON_HEIGHT),
            text="Passa il turno",
            container=actions_UI,
            object_id="PASS",
            manager=self.manager,
    ...
    def draw_leaderboard(self, players, squareBalance, currentPlayer):
        ...
        self.turnName = UILabel(
            turn_rect, "Turno di " + currentPlayer.get_name(), manager=self.manager, container=leaderboard
        )
        # ADDING SQUARE BALANCE LABEL
        balance_rect = pygame.Rect((position_x, 50), label_dimension)
        self.squareBalanceLabel = UILabel(
            balance_rect, "Riserva di piazza : " + str(squareBalance), manager=self.manager, container=leaderboard
        )
        ...
        for i, player in enumerate(players):  # considerare di fare una lable unica e andare a capo per ogni riga
            ...
            label = UILabel(
                player_label_rect,
                player.get_name() + " : " + str(player.get_balance()) + " | " + str(player.stock_value()),
                manager=self.manager,
        ...
        for i in range(start_range, end_range):
            ...
            if len(player.get_stocks()) == 0:
                player_label_rect = pygame.Rect(
                    (position_x, offset + 20 + CELL_HEIGHT + (max_stock * 20)), label_dimension
                )
                ...
                for stock in player.get_stocks():  # considerare di fare una lable unica e andare a capo per ogni riga
                    ...
                    if latest_stock_position == stock.get_position():
                        ...
                        stockNameLabel = UILabel(player_label_rect, stock.get_name(), manager=self.manager)
                        count_label += 1
    ...
    def showBuyAnythingStock(self, stocks, title):
        ...
        buyRect = pygame.Rect(
            (STOCK_UI_WIDTH - STOCK_UI_BUT_WIDTH - 10, STOCK_UI_HEIGHT - STOCK_UI_BUT_HEIGHT - 10),
            (STOCK_UI_BUT_WIDTH, STOCK_UI_BUT_HEIGHT),
        )

        self.buyAnyBut = UIButton(
            relative_rect=buyRect,
    ...
    def showChooseStock(self, stocks, title):
        ...
        self.showedStock = 0
        self.drawStockUi(title, True)
        chooseRect = pygame.Rect(
            (STOCK_UI_WIDTH - STOCK_UI_BUT_WIDTH - 10, STOCK_UI_HEIGHT - STOCK_UI_BUT_HEIGHT - 10),
            (STOCK_UI_BUT_WIDTH, STOCK_UI_BUT_HEIGHT),
        )

        self.chooseBut = UIButton(
            relative_rect=chooseRect,
            text="Scegli",
            container=self.stocksUi,
    ...
    def drawStockUi(self, title, close):
        ...
        title_rect = pygame.Rect(
            (STOCK_UI_WIDTH // 2 - STOCK_UI_TITLE_WIDTH // 2, 10), (STOCK_UI_TITLE_WIDTH, STOCK_UI_TITLE_HEIGHT)
        )
        UILabel(title_rect, title, manager=self.manager, container=self.stocksUi)

        nextRect = pygame.Rect(
            (STOCK_UI_WIDTH - 30 - STOCK_UI_BUT_WIDTH, STOCK_UI_HEIGHT // 2 - STOCK_UI_BUT_HEIGHT // 2),
            (STOCK_UI_BUT_WIDTH, STOCK_UI_BUT_HEIGHT),
        )
        prevRect = pygame.Rect(
            (30, STOCK_UI_HEIGHT // 2 - STOCK_UI_BUT_HEIGHT // 2), (STOCK_UI_BUT_WIDTH, STOCK_UI_BUT_HEIGHT)
        )
        closeRect = pygame.Rect((STOCK_UI_WIDTH - STOCK_UI_BUT_WIDTH, 0), (STOCK_UI_BUT_WIDTH, STOCK_UI_BUT_HEIGHT))

        self.nextStock = UIButton(
            relative_rect=nextRect, text=">", container=self.stocksUi, object_id="NEXT_STOCK", manager=self.manager
        )

        self.previousStock = UIButton(
            relative_rect=prevRect, text="<", container=self.stocksUi, object_id="PREV_STOCK", manager=self.manager
        )
        if close:
            self.closeStock = UIButton(
                relative_rect=closeRect,
                text="X",
                container=self.stocksUi,
    ...
    def drawEventUi(self):
        ...
        title_rect = pygame.Rect(
            (EVENT_UI_WIDTH // 2 - EVENT_UI_TITLE_WIDTH // 2, 10), (EVENT_UI_TITLE_WIDTH, EVENT_UI_TITLE_HEIGHT)
        )
        UILabel(title_rect, "EVENTI", manager=self.manager, container=self.eventUi)

        eventRect = pygame.Rect(
            (EVENT_UI_WIDTH - 30 - EVENT_UI_BUT_WIDTH, EVENT_UI_HEIGHT // 2 - EVENT_UI_BUT_HEIGHT // 2),
            (EVENT_UI_BUT_WIDTH, EVENT_UI_BUT_WIDTH),
        )

        self.eventBut = UIButton(
            relative_rect=eventRect, text="OK", container=self.eventUi, object_id="EVENT_OK", manager=self.manager
        )
        ...
        self.eventImage = UIImage(
            eventImageRect, self.showedEvent.surface, container=self.eventUi, manager=self.manager
        )
    ...
    def drawAlert(self, message):
        ...
        message_rect = pygame.Rect(
            ((ALERT_WIDTH - ALERT_MESSAGE_WIDTH) // 2, (ALERT_HEIGHT - ALERT_MESSAGE_HEIGHT - 20) // 2),
            (ALERT_MESSAGE_WIDTH, ALERT_MESSAGE_HEIGHT),
        )
        UILabel(message_rect, message, manager=self.manager, container=self.alertUi)

        close_rect = pygame.Rect(
            ((ALERT_WIDTH - ALERT_BUT_WIDTH) // 2, ALERT_HEIGHT - ALERT_BUT_HEIGHT - 10),
    ...
    def drawDiceOverlay(self, message, title, twoDices=True):
        ...
        title_rect = pygame.Rect(
            ((DICE_OVERLAY_WIDTH - ALERT_MESSAGE_WIDTH) // 2, 10), (ALERT_MESSAGE_WIDTH, ALERT_MESSAGE_HEIGHT)
        )
        UILabel(title_rect, title, manager=self.manager, container=self.diceOverlay)

        message_rect = pygame.Rect(
            ((DICE_OVERLAY_WIDTH - ALERT_MESSAGE_WIDTH) // 2, 30), (ALERT_MESSAGE_WIDTH, ALERT_MESSAGE_HEIGHT)
        )
        ...
        close_rect = pygame.Rect(
            (DICE_OVERLAY_WIDTH - ALERT_BUT_WIDTH - 20, DICE_OVERLAY_HEIGHT - ALERT_BUT_HEIGHT - 10),
            (ALERT_BUT_WIDTH, ALERT_BUT_HEIGHT),
        )
