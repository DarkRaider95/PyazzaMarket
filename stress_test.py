"""
Stress tester per PyazzaMarket.
Esegue partite automatiche complete con logging dettagliato per trovare bug.
"""
...
class StressTestRunner:
    """Esegue stress test automatici del gioco."""

    def __init__(self, num_tests: int = 1, seed: Optional[int] = None,
                 max_turns: int = 500, headless: bool = True):
        """
        ...
        Args:
            ...
            headless: Se True, esegue senza GUI (più veloce)
        """
    ...
    def run_all_tests(self):
        ...
        print(f"╔{'═'*78}╗")
        print(f"║ PyazzaMarket Stress Test Runner{' '*45}║")
    ...
    def run_single_test(self, seed: int, test_num: int) -> Dict[str, Any]:
        ...
        # Crea giocatori bot - usa le costanti per i path delle macchine
        from lib.constants import CAR_RED, CAR_BLUE, CAR_BLACK, CAR_YELLOW
    ...
    def create_test_game(self, width: int, height: int, clock, players: List[Dict],
        ...
        """
        # Crea il gioco sempre con GUI (il parametro gui=False non è ancora supportato completamente)
        # TODO: in futuro migliorare il supporto headless nel Game
        # IMPORTANTE: disabilitiamo il bot integrato perché lo stress test usa l'auto-clicker
    ...
    def run_game_automated(self, game: Game, logger: GameLogger, max_turns: int) -> int:
        ...
        """
        # Usa un thread separato per simulare i click automatici
        import threading
        import time as time_module

        turn_count = [0]  # Lista per modificarla nel thread
        game_started = [False]  # Flag per sapere quando il gioco è iniziato
        max_turns_reached = [False]

        def auto_clicker():
            """Thread che simula i click automatici."""
            time_module.sleep(0.5)  # Aspetta che la UI sia pronta

            # Fase 1: Gestisci il dice overlay iniziale per ordine turni
            # Sequenza: lancia -> chiudi -> lancia -> chiudi ... finché tutti non hanno tirato
            dice_launched = [False]  # Tiene traccia se abbiamo appena lanciato

            while not game_started[0] and game.running:
                ...
                # Se overlay_on() è False, tutti hanno tirato, chiudo l'overlay definitivamente
                if not game.dice_overlay.overlay_on():
                ...
                # Se non abbiamo lanciato, lancia
                if not dice_launched[0]:
                    if hasattr(game.dice_overlay, 'launchOverlayDiceBut'):
                        ...
                        pygame.event.post(click_event)
                        dice_launched[0] = True
                        time_module.sleep(0.3)
                # Se abbiamo lanciato, chiudi per passare al prossimo
                else:
                    if hasattr(game.dice_overlay, 'closeDiceOverlayBut'):
                        ...
                        pygame.event.post(click_event)
                        dice_launched[0] = False  # Reset per il prossimo giocatore
                        time_module.sleep(0.3)

            # Fase 2: Gioca i turni
            while turn_count[0] < max_turns and len(game.get_players()) > 1 and game.running:
                time_module.sleep(0.15)

                # PRIORITÀ 1: Gestisci pannelli aperti prima di tutto
                if game.current_panel is not None:
                    # Pannello alert - verifica che current_panel sia effettivamente l'alertUi
                    ...
                    # Pannello evento con pulsante conferma
                    ...
                    # Dice overlay durante il gioco (es. riserva monetaria) - lancia e chiudi
                    ...
                    # ShowStockUI - gestisci i vari tipi
                    # SHOW_CHOOSE_STOCK (fermata libera), MOVE_TO_STOCK
                    if hasattr(game.current_panel, 'chooseBut'):
                        # Aspetta che il pannello sia completamente disegnato
                        time_module.sleep(0.3)
                        stock_name = game.current_panel.get_showed_stock().get_name() if hasattr(game.current_panel, 'get_showed_stock') else "unknown"

                        # Decidi se comprare basandosi sul saldo
                        ...
                        if stock_price == 0 or player_balance > stock_price * 5:
                            buy_probability = 0.8  # Molto conveniente
                        elif player_balance > stock_price * 3:
                            buy_probability = 0.6  # Conveniente
                        elif player_balance > stock_price * 1.5:
                            buy_probability = 0.4  # Poco conveniente
                        elif player_balance > stock_price:
                            buy_probability = 0.2  # Molto costoso
                        else:
                        ...
                        if random_value < buy_probability:
                            logger.log_action("bot", "choose_stock", {"stock": stock_name, "panel_type": getattr(game.current_panel, 'type', 'unknown'), "price": stock_price, "balance": player_balance})
                            click_event = pygame.event.Event(
                                pygame_gui.UI_BUTTON_PRESSED,
                            ...
                            # Non comprare, chiudi il pannello
                    ...
                    # BANKRUPT_STOCK - metti all'asta
                    if hasattr(game.current_panel, 'auction_bankrupt'):
                        stock_name = game.current_panel.get_showed_stock().get_name() if hasattr(game.current_panel, 'get_showed_stock') else "unknown"
                        logger.log_action("bot", "bankrupt_auction", {"stock": stock_name})
                        click_event = pygame.event.Event(
                            pygame_gui.UI_BUTTON_PRESSED,
                    ...
                    if hasattr(game.current_panel, 'buyAnyBut'):
                        ...
                        buy_probability = 0.4  # Un po' più basso perché è "compra qualsiasi"
                        if stock_price == 0 or player_balance > stock_price * 5:
                            buy_probability = 0.7
                        elif player_balance > stock_price * 3:
                            buy_probability = 0.5
                    ...
                    # Auction - gestisci con scelte random ma sensate
                    if hasattr(game.current_panel, 'retireAuction'):
                        ...
                        if current_player_in_auction:
                            ...
                            # Traccia quante volte questo giocatore ha passato
                            ...
                            if game.current_panel._bot_auction_pass_count[player_name] >= 2:
                                ...
                                # Decidi se ritirarsi definitivamente dall'asta
                                # Ritirati se il prezzo è troppo alto o se hai pochi soldi
                                retire_probability = 0.0
                                if player_balance < current_bid * 1.1:
                                    retire_probability = 0.7  # Molto costoso, alta probabilità di ritirarsi
                                elif player_balance < current_bid * 1.5:
                                    retire_probability = 0.4  # Abbastanza costoso
                                elif player_balance < current_bid * 2.5:
                                    retire_probability = 0.2  # Ancora gestibile
                                else:
                                    retire_probability = 0.05  # Probabilità base di ritirarsi

                                # Aumenta probabilità di ritirarsi se è l'unico rimasto o se ha già passato una volta
                                if active_bidders <= 2:
                                    retire_probability += 0.15
                                if game.current_panel._bot_auction_pass_count[player_name] >= 1:
                                    retire_probability += 0.25

                                # Calcola probabilità di fare offerta
                                bid_probability = 0.0
                                if current_bid == 0:
                                    bid_probability = 0.6  # Prima offerta
                                elif player_balance > current_bid * 4:
                                    bid_probability = 0.5  # Molto conveniente
                                elif player_balance > current_bid * 2:
                                    bid_probability = 0.3  # Abbastanza conveniente
                                elif player_balance > current_bid * 1.3:
                                    bid_probability = 0.15  # Poco conveniente

                                # Usa il seeded random per decidere
                                random_value = game.dice_controller.seeded_random.rng.random()

                                # Prima controlla se ritirarsi (priorità alta se troppo costoso)
                                if random_value < retire_probability:
                                    action = 'retire'
                                # Poi controlla se fare offerta
                                elif random_value < retire_probability + bid_probability:
                                    action = 'bid'
                                # Altrimenti passa il turno
                                else:
                                    action = 'pass'
                                    # Incrementa il contatore di pass per questo giocatore
                                    game.current_panel._bot_auction_pass_count[player_name] += 1

                        if action == 'bid' and hasattr(game.current_panel, 'bidBut'):
                            # Reset pass count quando fai un'offerta
                            if current_player_in_auction:
                    ...
                    # BargainUI - chiudi senza contrattare
                    ...
                    # TakeSomeoneWithYouUI - scegli giocatore
                ...
                if game._Game__actions_status.get_buy_property():
                    ...
                    # Ottieni informazioni sullo stock
                    ...
                    if current_cell.get_stocks() is not None and len(current_cell.get_stocks()) > 0:
                        ...
                        # Calcola probabilità di acquisto basata su condizioni economiche
                        buy_probability = 0.6  # Base: 60% probabilità

                        # Aumenta probabilità se ha poche cedole (diversificazione)
                        if num_stocks < 3:
                            buy_probability += 0.2
                        elif num_stocks < 5:
                            buy_probability += 0.1

                        # Aggiusta in base al rapporto prezzo/saldo
                        if stock_price == 0:
                            buy_probability = 1.0  # Stock gratis, compra sempre
                        elif player_balance > stock_price * 5:
                            buy_probability += 0.2  # Molto conveniente
                        elif player_balance > stock_price * 3:
                            buy_probability += 0.1  # Conveniente
                        elif player_balance > stock_price * 1.5:
                            buy_probability -= 0.1  # Poco conveniente
                        elif player_balance > stock_price:
                            buy_probability -= 0.3  # Costoso
                        else:
                            buy_probability = 0.0  # Non può permetterselo

                        # Diminuisci probabilità se ha molte cedole
                        if num_stocks > 8:
                            buy_probability -= 0.2
                        elif num_stocks > 6:
                            buy_probability -= 0.1

                        # Assicurati che sia tra 0 e 1
                        buy_probability = max(0.0, min(1.0, buy_probability))

                        # Decidi se comprare usando random seeded
                        random_value = game.dice_controller.seeded_random.rng.random()

                        if random_value < buy_probability and hasattr(game._Game__gameUI, 'buyButton'):
            ...
            if turn_count[0] >= max_turns:
                max_turns_reached[0] = True
                game.running = False

        # Avvia thread auto-clicker
        clicker_thread = threading.Thread(target=auto_clicker, daemon=True)
        clicker_thread.start()

        # Avvia il gioco normale (usa il loop di start())
        try:
            game.start()
        except Exception as e:
            # Log dell'errore
            logger.log_error(
                type(e).__name__,
                f"Errore al turno {turn_count[0]}: {str(e)}",
                traceback.format_exc(),
                self.get_game_state(game)
            )
            raise
    ...
    def execute_automated_turn(self, game: Game, logger: GameLogger):
        """
        Esegue un turno automatico usando la logica del bot.
        ...
        """
        current_player = game.get_current_player()
        position_before = current_player.get_position()

        # Usa il dice controller se disponibile
        if hasattr(game, 'dice_controller'):
            dice_roll = game.dice_controller.roll()
            game._Game__test = True
            game._Game__test_dice = dice_roll
        else:
            dice_roll = (0, 0)  # Verrà generato casualmente

        # Log del lancio
        if dice_roll != (0, 0):
            logger.log_dice_roll(
                current_player.get_name(),
                dice_roll[0],
                dice_roll[1],
                position_before,
        ...
        # Esegui il turno effettivo
        # Questo chiama la logica del bot se il giocatore è un bot
        game.turn()

        # Log del movimento
        position_after = current_player.get_position()
        if position_after != position_before:
            passed_start = position_after < position_before
        ...
        # Simula l'interazione automatica per chiudere pannelli, eventi, etc
        self.auto_interact_with_ui(game, logger)

    def auto_interact_with_ui(self, game: Game, logger: GameLogger):
        """
        Simula l'interazione automatica con UI panels ed eventi.

        Args:
            game: Istanza del gioco
            logger: Logger
        """
        # Chiudi alert messages automaticamente
        if hasattr(game, '_Game__alert_messages') and game._Game__alert_messages:
            while game._Game__alert_messages:
                game._Game__alert_messages.pop(0)

        # Gestisci panels aperti automaticamente
        if hasattr(game, 'current_panel') and game.current_panel is not None:
            # Simula click su pulsanti di chiusura o azioni automatiche
            # Questo dipende dal tipo di panel
            pass

        # Passa il turno se necessario
        if game.get_actions_status().can_pass_turn():
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Stress tester per PyazzaMarket",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
Esempi d'uso:
  # Esegui 10 test con seed random
  python stress_test.py -n 10

  # Esegui 5 test con un seed specifico
  python stress_test.py -n 5 --seed 12345

  # Esegui 100 test con GUI disabilitata (più veloce)
  python stress_test.py -n 100 --headless

  # Esegui test con limite di 200 turni
  python stress_test.py -n 10 --max-turns 200
        """
    )
    ...
    parser.add_argument(
        "--headless",
        action="store_true",
        help="Esegui senza GUI (molto più veloce)"
    )
