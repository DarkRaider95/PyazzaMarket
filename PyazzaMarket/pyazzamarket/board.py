...
class Board:
    ...
    def createBottomTriplet(self, curr_x, curr_y, color, eventCell, eventImage):
        ...
        for i in range(0, 4):
            ...
            if(i == eventCell):
                cell = Cell(None, None, None, x, y, eventImage)
                self.cells.append(cell)
            else:
                cell = Cell(cellDef['color'], cellDef['value'], None, x, y, None)
                self.cells.append(cell)
