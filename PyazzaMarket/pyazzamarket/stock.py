...
class Stock:
    ...
    def __init__(self, color, stock_value, values, logo, position):
        ...
        self.font_prices = pygame.font.Font(None, 32)

    def draw(self, screen):
        #Draw stock and spaces for logo and prices
        ...
        pricerect = pygame.Rect(price_x, price_y, PRICE_WIDTH, PRICES_HEIGHT)
        pygame.draw.rect(screen, self.color, rect)
        pygame.draw.rect(screen, WHITE, logorect)
        pygame.draw.rect(screen, WHITE, pricerect)

        #Draw stock price and prices
        ...
        for i, value in enumerate(self.values):
            #write string value
            string_value = self.font_prices.render(Stock.string_values[i], True, BLACK)
            screen.blit(string_value, (price_x + 10, price_y + 30 * i + 1))
            #write fee value for that string
            value_price = self.font_prices.render(str(self.values[i]) + " SCUDI", True, BLACK)
            screen.blit(value_price, (price_x + PRICE_WIDTH - value_price.get_width() - 10, price_y + 30 * i + 1))
